...
Bubbles:
The bubbles originate from the diver's head in groups. I decided to create them in random groups of 3 - 5. The timing between
each group is also random within a certain range. When they are generated, the current location of
the diver's head is stored in an array. The bubbles are then rendered in the ground box coordinate system, and move up
at a constant rate. They are deleted when they reach the top of the ground box. The bubbles also modulate subtly in the x
and y direction. Since the bubbles are not drawn in the diver's coordinate system, they are not tied to the motion of the
diver. Therefore, if the diver were to move, the bubbles would not move with them.

Overall, I was happy to be able to accomplish everything listed in the rubric.